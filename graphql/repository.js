...
const withIssues = graphql(GetRepositoryIssuesQuery, {
  ...
  props: ({ data }) => {
            ...
            repositoryOwner: {
              repository: {
                issues: {
                  ...
                  edges: [
                    ...fetchMoreResult.data.repositoryOwner.repository.issues
                      .edges,
    ...
    return {
      ...
      issues: [
        ...data.repositoryOwner.repository.issues.edges.map(({ node }) => node)
      ].reverse(),
      hasNextPage:
        data.repositoryOwner.repository.issues.pageInfo.hasPreviousPage,
      fetchNextPage
    };
  }
});
          ...
          renderRow={issue => {
            return (
              <TouchableHighlight
                activeOpacity={50}
                underlayColor="blue"
                onPress={() => goToIssue(issue.id, issue.title)}
              >
